...
async def async_refresh_saving_sessions(
  ...
  if existing_saving_sessions_result is None or (existing_saving_sessions_result.last_retrieved + timedelta(minutes=30)) < current or client.is_time_for_half_hourly_call(current):
    try:
